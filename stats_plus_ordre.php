    ...
    <div id="bodyMiddlePan">
        ...
        if (!empty($_SESSION['connecte'])) {
            ...
            if (isset($_SESSION['nom'])) {
                ...
                echo"<fieldset>";
                echo"<font color='black' size=4><b>Auteur avec le plus d'articles en tant qu'auteur n°1</b></font><br/><br/>";
                plus_grand_contributeur_ordre1($mabase);
...
<?php
function plus_grand_contributeur_ordre1($mabase) {
    //compteur d'auteurs
    ...
    $nombre_publications_ancien_auteur = '0';
    $compteur_auteur_similaire = 1;
    $auteur = 'auteur';
    for ($compteur = 0; $compteur < $nbr_auteurs; $compteur++) {
        $query = "SELECT count(auteur_id) as nbr FROM publications WHERE auteur_id=$compteur AND ordre=1";
        $resultat = mysqli_query($mabase, $query);
        while ($ligne = mysqli_fetch_array($resultat, MYSQLI_ASSOC)) {
    ...
    echo("Le(s) auteur(s) ayant écrit le plus d'articles ($nombre_publications_ancien_auteur) en temps qu'auteurs n°1 sont :<br/><br/>");
    echo("<ul>");
