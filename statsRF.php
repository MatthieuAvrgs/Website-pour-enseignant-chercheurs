    ...
    <div id="bodyMiddlePan">
        ...
        if (!empty($_SESSION['connecte'])) {
            ...
            if (isset($_SESSION['nom'])) {
                echo"<fieldset>";
                echo"<font color='black' size=4><b>Articles RF</b></font><br/><br/>";
                ...
                $queryRF = "SELECT * FROM articles WHERE categorie = 'RF'";
                $resultatRF = mysqli_query($mabase, $queryRF);
                if (mysqli_num_rows($resultatRF) == 0) {
                    echo "Il n'y a aucun article dans la categorie Revue Française <br/> <br/>";
                    echo "<a href='statistiques.php'>Retour</a>";
                } else {
                    $i = 0;

                    //a chaque fois que un auteur a écris un RF, on incrémente la valeur dont la clé correspond à l'id
                    while ($ligneRF = mysqli_fetch_array($resultatRF, MYSQLI_ASSOC)) {
                        $query_publicationsRF = "SELECT article_id, auteur_id FROM publications WHERE article_id =" . $ligneRF['id'] . ";";
                        $resultat_publicationsRF = mysqli_query($mabase, $query_publicationsRF);
                        $compteur = 0;
                        while ($ligne_publicationsRF = mysqli_fetch_array($resultat_publicationsRF, MYSQLI_ASSOC)) {
                            for ($compteur = 0; $compteur <= $nbr_auteurs; $compteur++) {
                                if ($ligne_publicationsRF['auteur_id'] == $compteur) {
                    ...
                    $maxvalue = array ($nb_publications[0]);
                    $maxid = array (0);
                    ...
                    echo "Le(s) auteur(s) ayant écrit le plus d'articles de Revue Française (".$max.") sont : <br/><br/>";
                    echo "<ul>";
                    $numero = 0;
