    ...
    <div id="bodyMiddlePan">
        ...
        if (!empty($_SESSION['connecte'])) {
            ...
            if (isset($_SESSION['nom'])) {
                echo"<fieldset>";
                echo"<font color='black' size=4><b>Articles CF</b></font><br/><br/>";
                ...
                //echo "<br/><br/><br/>";
                $queryCF = "SELECT * FROM articles WHERE categorie = 'CF'";
                $resultatCF = mysqli_query($mabase, $queryCF);
                if (mysqli_num_rows($resultatCF) == 0) {
                    echo "Il n'y a aucun article dans la catégorie Conférences Françaises <br/> <br/>";
                    echo "<a href='statistiques.php'>Retour</a>";
                } else {
                    $i = 0;

                    //a chaque fois que un auteur a écris un CF, on incrémente la valeur dont la clé correspond à l'id
                    while ($ligneCF = mysqli_fetch_array($resultatCF, MYSQLI_ASSOC)) {
                        $query_publicationsCF = "SELECT article_id, auteur_id FROM publications WHERE article_id =" . $ligneCF['id'] . ";";
                        $resultat_publicationsCF = mysqli_query($mabase, $query_publicationsCF);
                        $compteur = 0;
                        while ($ligne_publicationsCF = mysqli_fetch_array($resultat_publicationsCF, MYSQLI_ASSOC)) {
                            for ($compteur = 0; $compteur <= $nbr_auteurs; $compteur++) {
                                if ($ligne_publicationsCF['auteur_id'] == $compteur) {
                    ...
                    echo "Le(s) auteur(s) ayant écrit le plus d'articles de Conférences Françaises (".$max.") sont : <br/><br/>";
                    echo "<ul>";
                    $numero = 0;
