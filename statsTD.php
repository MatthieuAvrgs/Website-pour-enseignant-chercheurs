    ...
    <div id="bodyMiddlePan">
        ...
        if (!empty($_SESSION['connecte'])) {
            ...
            if (isset($_SESSION['nom'])) {
                echo"<fieldset>";
                echo"<font color='black' size=4><b>Articles TD</b></font><br/><br/>";
                ...
                $queryTD = "SELECT * FROM articles WHERE categorie = 'TD'";
                $resultatTD = mysqli_query($mabase, $queryTD);
                if (mysqli_num_rows($resultatTD) == 0) {
                    echo "Il n'y a aucun article dans la catégorie Thèses de Doctorat <br/> <br/>";
                    echo "<a href='statistiques.php'>Retour</a>";
                } else {
                    $i = 0;

                    //a chaque fois que un auteur a écris un TD, on incrémente la valeur dont la clé correspond à l'id
                    while ($ligneTD = mysqli_fetch_array($resultatTD, MYSQLI_ASSOC)) {
                        $query_publicationsTD = "SELECT article_id, auteur_id FROM publications WHERE article_id =" . $ligneTD['id'] . ";";
                        $resultat_publicationsTD = mysqli_query($mabase, $query_publicationsTD);
                        $compteur = 0;
                        while ($ligne_publicationsTD = mysqli_fetch_array($resultat_publicationsTD, MYSQLI_ASSOC)) {
                            for ($compteur = 0; $compteur <= $nbr_auteurs; $compteur++) {
                                if ($ligne_publicationsTD['auteur_id'] == $compteur) {
                    ...
                    echo "Le(s) auteur(s) ayant écrit le plus de Thèses de Doctorat (".$max.") sont : <br/><br/>";
                    echo "<ul>";
                    $numero = 0;
