    ...
    <div id="bodyMiddlePan">
        ...
        if (!empty($_SESSION['connecte'])) {
            ...
            if (isset($_SESSION['nom'])) {
                echo"<fieldset>";
                echo"<font color='black' size=4><b>Articles CI</b></font><br/><br/>";
                ...
                $queryCI = "SELECT * FROM articles WHERE categorie = 'CI'";
                $resultatCI = mysqli_query($mabase, $queryCI);
                if (mysqli_num_rows($resultatCI) == 0) {
                    echo "Il n'y a aucun article dans la catégorie Conférences Internationales <br/> <br/>";
                    echo "<a href='statistiques.php'>Retour</a>";
                } else {
                    $i = 0;

                    //a chaque fois que un auteur a écris un CI, on incrémente la valeur dont la clé correspond à l'id
                    while ($ligneCI = mysqli_fetch_array($resultatCI, MYSQLI_ASSOC)) {
                        $query_publicationsCI = "SELECT article_id, auteur_id FROM publications WHERE article_id =" . $ligneCI['id'] . ";";
                        $resultat_publicationsCI = mysqli_query($mabase, $query_publicationsCI);
                        $compteur = 0;
                        while ($ligne_publicationsCI = mysqli_fetch_array($resultat_publicationsCI, MYSQLI_ASSOC)) {
                            for ($compteur = 0; $compteur <= $nbr_auteurs; $compteur++) {
                                if ($ligne_publicationsCI['auteur_id'] == $compteur) {
                    ...
                    echo "Le(s) auteur(s) ayant écrit le plus d'articles de Conférences Internationales (".$max.") sont : <br/><br/>";
                    echo "<ul>";
                    $numero = 0;
