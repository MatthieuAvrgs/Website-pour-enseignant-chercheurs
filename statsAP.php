    ...
    <div id="bodyMiddlePan">
        ...
        if (!empty($_SESSION['connecte'])) {
            ...
            if (isset($_SESSION['nom'])) {
                echo"<fieldset>";
                echo"<font color='black' size=4><b>Articles AP</b></font><br/><br/>";
                ...
                //echo "<br/><br/><br/>";
                $queryAP = "SELECT * FROM articles WHERE categorie = 'AP'";
                $resultatAP = mysqli_query($mabase, $queryAP);
                if (mysqli_num_rows($resultatAP) == 0) {
                    echo "Il n'y a aucun article dans la catégorie Autres Productions <br/> <br/>";
                    echo "<a href='statistiques.php'>Retour</a>";
                } else {
                    $i = 0;

                    //a chaque fois que un auteur a écris un AP, on incrémente la valeur dont la clé correspond à l'id
                    while ($ligneAP = mysqli_fetch_array($resultatAP, MYSQLI_ASSOC)) {
                        $query_publicationsAP = "SELECT article_id, auteur_id FROM publications WHERE article_id =" . $ligneAP['id'] . ";";
                        $resultat_publicationsAP = mysqli_query($mabase, $query_publicationsAP);
                        $compteur = 0;
                        while ($ligne_publicationsAP = mysqli_fetch_array($resultat_publicationsAP, MYSQLI_ASSOC)) {
                            for ($compteur = 0; $compteur <= $nbr_auteurs; $compteur++) {
                                if ($ligne_publicationsAP['auteur_id'] == $compteur) {
                    ...
                    echo "Le(s) auteur(s) ayant écrit le plus d'articles Autres Productions (".$max.") sont : <br/><br/>";
                    echo "<ul>";
                    $numero = 0;
