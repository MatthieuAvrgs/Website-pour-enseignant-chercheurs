    ...
    <div id="bodyMiddlePan">
        ...
        if (!empty($_SESSION['connecte'])) {
            ...
            if (isset($_SESSION['nom'])) {
                echo"<fieldset>";
                echo"<font color='black' size=4><b>Articles RI</b></font><br/><br/>";
                //connexion base
                ...
                $queryRI = "SELECT * FROM articles WHERE categorie = 'RI'";
                $resultatRI = mysqli_query($mabase, $queryRI);
                if (mysqli_num_rows($resultatRI) == 0) {
                    echo "Il n'y a aucun article dans la catégorie Revue Internationale<br/><br/>";
                    echo "<a href='statistiques.php'>Retour</a>";
                } else {
                    $i = 0;

                    //a chaque fois que un auteur a écris un RI, on incrémente la valeur dont la clé correspond à l'id
                    while ($ligneRI = mysqli_fetch_array($resultatRI, MYSQLI_ASSOC)) {
                        $query_publicationsRI = "SELECT article_id, auteur_id FROM publications WHERE article_id =" . $ligneRI['id'] . ";";
                        $resultat_publicationsRI = mysqli_query($mabase, $query_publicationsRI);
                        $compteur = 0;
                        while ($ligne_publicationsRI = mysqli_fetch_array($resultat_publicationsRI, MYSQLI_ASSOC)) {
                            for ($compteur = 0; $compteur <= $nbr_auteurs; $compteur++) {
                                if ($ligne_publicationsRI['auteur_id'] == $compteur) {
                    ...
                    echo "Le(s) auteur(s) ayant écrit le plus d'articles de Revue Internationale (".$max.") sont : <br/><br/>";
                    echo "<ul>";
                    $numero = 0;
